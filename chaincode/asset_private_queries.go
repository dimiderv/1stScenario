...
// ReadRequestToBuy gets the buyer's identity from the transfer agreement from world state
func (s *SmartContract) ReadRequestToBuy(ctx contractapi.TransactionContextInterface, assetID string) (*RequestToBuyObject, error) {
	log.Printf("ReadRequestToBuy: collection %v, ID %v", assetCollection, assetID)
	// composite key for RequestToBuy of this asset
	requestToBuyKey, err := ctx.GetStub().CreateCompositeKey(requestToBuyObject, []string{assetID})
	if err != nil {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to read RequestToBuyKey: %v", err)
	}
	if buyerIdentity == nil {
		log.Printf("RequestToBuy for %v does not exist", assetID)
		return nil, nil
	}
