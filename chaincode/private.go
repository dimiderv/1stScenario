...
//Puts Price to owners implicit collection
...
func (s *SmartContract) RequestToBuy(ctx contractapi.TransactionContextInterface,assetID string ) error {
	...
	buyRequestKey, err := ctx.GetStub().CreateCompositeKey(requestToBuyObject, []string{assetID})
	if err != nil {
		return fmt.Errorf("failed to create composite key: %v", err)
	}
	// Will put Buy Request to World state
	err = ctx.GetStub().PutState(buyRequestKey, []byte(buyerID))
	if err != nil {
		return fmt.Errorf("failed to put asset bid: %v", err)
	}
...
func (s *SmartContract) TransferRequestedAsset(ctx contractapi.TransactionContextInterface) error {
	...
	buyRequest, err := s.ReadRequestToBuy(ctx, asset.ID)
	if err != nil {
...
//Deletes Bid request from buyers (now he is owner) implicit collection
