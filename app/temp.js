...
async function initContractFromOrg22Identity() {
    console.log('\n--> Fabric client user & Gateway init: Using Org2 identity to Org2 Peer');
    const ccpOrg2 = buildCCPOrg2();
    const caOrg2Client = buildCAClient(FabricCAServices, ccpOrg2, 'ca.org2.example.com');
    ...
    await enrollAdmin(caOrg2Client, walletOrg2, mspOrg2);
    await registerAndEnrollRetailer(caOrg2Client, walletOrg2, mspOrg2, "Farmerb", 'org2.department1');

    try {
        // Create a new gateway for connecting to Org's peer node.
        const gatewayOrg2 = new Gateway();
        await gatewayOrg2.connect(ccpOrg2,
            { wallet: walletOrg2, identity: "Farmerb", discovery: { enabled: true, asLocalhost: true } });
...
async function main() {
	try {
        ...
        const gatewayOrg11 = await initContractFromOrg22Identity();
        const networkOrg11 = await gatewayOrg11.getNetwork(channelName);
        const contractOrg11 = networkOrg11.getContract(chaincodeName);
		...
		try {
            ...
            result = await statefulTxn.submit(assetID,'green',10,"apple");
            ...
            // try to update from another same name user
			try {

				transaction = contractOrg11.createTransaction('UpdateAsset');
                transaction.setEndorsingOrganizations(mspOrg2);
				await transaction.submit(assetID,'red',100000);
				console.log(`*** Trying to update with same name but different Org shoudl throw error`);
			} catch (sellError) {
				console.log(`${RED}*** Failed: UpdateAsset - ${sellError}${RESET}`);
			}
			await sleep(2000);
